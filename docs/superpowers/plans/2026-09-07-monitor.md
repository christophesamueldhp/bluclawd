...
- [ ] **Step 1: Start pi from the working tree** (recipe above, session name `mon`).

- [ ] **Step 2: Ask the model to start a background job**
...
- [ ] **Step 4: Kill the session**: `tmux kill-session -t mon`.
...
- [ ] **Step 3: Move the registry onto `sharedRef`**
...
- [ ] **Step 1: Write the failing tests** (append; extend the import with `RateLimiter, tailOutput`)
...
- [ ] **Step 1: Write the failing tests** (append to `test/background-bash-registry.test.ts`)
...
- [ ] **Step 3: Implement in `ext/_shared/background-bash.ts`**
...
- [ ] **Step 1: Write the failing tests** (append; extend the import with `monitorEventMessage, monitorEndMessage, taskExitMessage`)
...
- [ ] **Step 1: Update the registration expectation**
...
- [ ] **Step 1: Write the failing test** (append; extend the import with `shouldNotifyExit`)
...
- [ ] **Step 3: Implement** (append to `ext/_shared/monitor-events.ts`)
...
No unit test: renderers are verified live in Task 11 (the repo's rule: TUI work is verified in tmux).
...
- [ ] **Step 2: Add kind and events to `/tasks`**
...
- [ ] **Step (b) event during a permission prompt**
...
- [ ] **Step (c) single notification from run_in_background**
...
- [ ] **Step (d) cross-extension registry**
...
- [ ] **Step (f) kill the session**: `tmux kill-session -t mon`.

- [ ] **Step (g) fix anything found, with a test where the defect is testable, and commit each fix separately.**
