...
| `/mcp` | MCP servers from `mcp.json` / `.mcp.json`; project servers need `/mcp approve` |
| `webfetch`, `websearch` | Claude Code's `WebFetch`/`WebSearch`. Fetch returns Markdown (tables included), decodes the page's charset, caches 15 minutes, blocks private addresses, and reports a redirect to another host instead of following it. Search works with no key (Exa's hosted endpoint), or with `EXA_API_KEY`/`BRAVE_API_KEY`/`TAVILY_API_KEY` (`settings.websearch.provider`); `allowed_domains`/`blocked_domains` narrow by site. Rules: `WebFetch(domain:example.com)` (what "Always allow" persists), `WebSearch(<query glob>)` |
...
| `/fleet` | session roster in the shape of Claude Code's `/resume` picker: title + `time · branch · N messages · path`, grouped Running / Saved with status glyphs, type to search, ctrl+a current/all projects, enter opens, ctrl+t peeks, ctrl+n starts one; restarts a stale daemon by itself when it owns no running session |
