...
- An extra entrance, `wave` (added 2026-09-25 at the user's request, reworked twice the same day): the hand rises in two steps while the eyes glance at it, then swings side to side three times like a human wave — a pendulum from the elbow, highest in the middle, over 4 pixels — and comes back down (27 frames, 1.6 s). The mascot box gained one blank cell on the right for the swing, and the gap to the text shrank to 1, so the text does not move.
