...
- An extra entrance, `wave` (added 2026-09-25 at the user's request, reworked twice the same day): the hand rises in two steps while the eyes glance at it, then swings side to side three times like a human wave — the arm stays joined to the body and is always its own solid 2×3 block (turned 3×2 when it points out), as thick as in mascot.svg: in beside the head, upright, out to the side — and comes back down (26 frames, 1.6 s). A test checks every frame of every sequence is one connected shape. The mascot box gained one blank cell on the right for the swing, and the gap to the text shrank to 1, so the text does not move.
